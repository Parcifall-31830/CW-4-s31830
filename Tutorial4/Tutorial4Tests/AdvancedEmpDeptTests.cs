...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnDistinctJobTitles()
    {
        ...
        var jobs = emps.Distinct().Select(e=>e.Job).ToList(); 
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(e=> new{EName=e.EName,Total=e.Sal+(e.Comm??0)}); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e=>e.DeptNo,d=>d.DeptNo,(e,d)=>new{Employee=e,Dept=d})
            .SelectMany(ed=>grades.Where(g=>ed.Employee.Sal>=g.Losal && ed.Employee.Sal<=g.Hisal).Select(g=>new{ed.Employee.EName,ed.Dept.DName,g.Grade})); 
